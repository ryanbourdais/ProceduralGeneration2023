...
public class TextureModifier : MonoBehaviour {
    ...
    public void ComponentRunner()
    {
        generateTextures(terrain);
    }
   
    public void generateTextures(Terrain terrainToChange) {

        terrain = terrainToChange;
        ...
        for (int i = 0; i < listTextures.Count; i++) {
            if (listTextures [i].defaultTexture) {
                indexOfTexture = listTextures [i].index;
            }
        }
        ...
        for (int y = 0; y < terrainData.alphamapHeight; y++)
        {
            for (int x = 0; x < terrainData.alphamapWidth; x++)
            {
                ...
                if(Mathf.Approximately(z, 0.0f)){
                    splatWeights [indexOfTexture] = 1.0f;
                }
